...
const columns: Column<AlertRow>[] = [
  ...
  },
  {
    ...
    render: (row) =>
      row.severity ? (
        <StatusChip label={row.severity} tone={row.severity === 'critical' || row.severity === 'high' ? 'blocked' : 'neutral'} />
      ) : (
  ...
  {
    key: 'activity',
    header: 'Occurrences / last seen',
    render: (row) => (
      <>
        {row.occurrences ?? 'Unknown'}
        <br />
        <small>{unknown(row.last_triggered_at)}</small>
      </>
    ),
  },
];
...
export default async function AlertsPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  return (
    <section className="content">
      <h1>Alerts</h1>
      <p className="muted">
        Repeat triggers increment an occurrence counter on one alert instead of creating duplicates, so a noisy rule
        cannot bury the rest of the queue.
      </p>
      <ResourceTable
