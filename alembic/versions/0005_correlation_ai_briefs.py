...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("primary_entity_type", sa.String(64), nullable=False),
    ...
    op.create_index(
        "ix_correlations_entity", "correlations", ["primary_entity_type", "primary_entity_id"]
    )
    op.create_index("ix_correlations_evaluated", "correlations", ["evaluated_at"])
    op.create_table(
        "correlation_ai_briefs",
        sa.Column("id", uuid, server_default=sa.text("uuid_generate_v4()"), nullable=False),
        ...
        sa.Column("model_label", sa.String(128), nullable=False, server_default="rag-grounded"),
        sa.Column(
        ...
        sa.PrimaryKeyConstraint("id", name="pk_correlation_ai_briefs"),
        sa.ForeignKeyConstraint(
            ["correlation_id"],
            ["correlations.id"],
            ondelete="CASCADE",
            name="fk_correlation_ai_briefs_correlation",
        ),
    )
    op.create_index(
        "ix_correlation_ai_briefs_correlation", "correlation_ai_briefs", ["correlation_id"]
    )
