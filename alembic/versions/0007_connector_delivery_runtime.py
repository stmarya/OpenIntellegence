...
def upgrade() -> None:
    op.add_column(
        "automation_outbox", sa.Column("attempts", sa.Integer(), nullable=False, server_default="0")
    )
    ...
    op.create_index(
        "ix_automation_outbox_available", "automation_outbox", ["state", "available_at"]
    )
    op.create_index(
        "ix_automation_outbox_lease", "automation_outbox", ["lease_token", "lease_until"]
    )
