...
BOOTSTRAP_SCOPES = [
    Scope.READ, Scope.WRITE, Scope.IOC, Scope.ENROLL,
    Scope.APIKEY_READ, Scope.APIKEY_WRITE, Scope.REPORT_WRITE, Scope.ADMIN,
]
...
async def bootstrap(slug: str, name: str, key_name: str) -> str:
    ...
    async with factory() as session:
        ...
        if tenant is None:
            ...
            session.add(tenant)
            await session.flush()
        existing = await session.scalar(select(func.count()).select_from(ApiKey).where(ApiKey.tenant_id == tenant.id))
        if existing:
            raise RuntimeError("Tenant already has an API key; bootstrap refuses to mint another.")
