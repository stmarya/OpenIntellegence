...
class AlertCreate(BaseModel):
    ...
    severity: Literal["low", "medium", "high", "critical"]; entity_type: str | None = Field(default=None, max_length=64); entity_id: str | None = Field(default=None, max_length=255)
    risk_score: int | None = Field(default=None, ge=0, le=100); payload: dict = Field(default_factory=dict)
class AlertOut(ORM):
...
async def create_rule(payload: AlertRuleCreate, db: DbSession, principal: WritePrincipal) -> AlertRuleOut:
    item = AlertRule(tenant_id=principal.tenant_id, **payload.model_dump()); db.add(item); await db.flush(); return AlertRuleOut.model_validate(item)
...
async def create_alert(payload: AlertCreate, db: DbSession, principal: WritePrincipal) -> AlertOut:
    ...
    now = datetime.now(UTC); fingerprint = _fingerprint(principal.tenant_id, payload, now)
    ...
    try:
        # The unique constraint is the concurrency boundary. A savepoint keeps the request transaction usable after a duplicate insert.
        async with db.begin_nested():
            db.add(item); await db.flush()
        return AlertOut.model_validate(item)
    except IntegrityError:
        existing = (await db.execute(select(Alert).where(Alert.tenant_id == principal.tenant_id, Alert.fingerprint == fingerprint).with_for_update())).scalar_one()
        existing.occurrences += 1; existing.last_triggered_at = now; existing.payload = payload.payload; await db.flush(); return AlertOut.model_validate(existing)
