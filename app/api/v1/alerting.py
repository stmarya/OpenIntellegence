...
def _fingerprint(tenant_id: str, payload: AlertCreate, bucket: datetime) -> str:
    """Stable per-hour deduplication key for a semantically identical alert."""
    raw = "|".join((tenant_id, payload.rule_id or "manual", payload.entity_type or "", payload.entity_id or "", payload.severity, bucket.strftime("%Y%m%d%H")))
    return sha256(raw.encode()).hexdigest()


@router.get("/alert-rules", response_model=ListResponse[AlertRuleOut], summary="List alert rules")
async def list_rules(db: DbSession, principal: ReadPrincipal, limit: Annotated[int, Query(ge=1, le=200)] = 50, offset: Annotated[int, Query(ge=0)] = 0) -> ListResponse[AlertRuleOut]:
    ...
    rows = (await db.execute(stmt.order_by(AlertRule.created_at.desc()).limit(limit).offset(offset))).scalars().all()
    return ListResponse(data=[AlertRuleOut.model_validate(x) for x in rows], page=Page(limit=limit, offset=offset, total=total, has_more=offset + limit < total), provenance=await build_provenance(db, sources=None))


@router.post("/alert-rules", response_model=AlertRuleOut, status_code=status.HTTP_201_CREATED, summary="Create rule-driven alert policy")
...
@router.get("/alerts", response_model=ListResponse[AlertOut], summary="List prioritized alerts")
async def list_alerts(db: DbSession, principal: ReadPrincipal, limit: Annotated[int, Query(ge=1, le=500)] = 50, offset: Annotated[int, Query(ge=0)] = 0, alert_status: Annotated[str | None, Query(alias="status")] = None, severity: str | None = None) -> ListResponse[AlertOut]:
    ...
    total = await db.scalar(select(func.count()).select_from(stmt.subquery())) or 0
    rows = (await db.execute(stmt.order_by(Alert.risk_score.desc().nulls_last(), Alert.last_triggered_at.desc()).limit(limit).offset(offset))).scalars().all()
    return ListResponse(data=[AlertOut.model_validate(x) for x in rows], page=Page(limit=limit, offset=offset, total=total, has_more=offset + limit < total), provenance=await build_provenance(db, sources=None))


@router.post("/alerts", response_model=AlertOut, status_code=status.HTTP_201_CREATED, summary="Record or deduplicate an alert occurrence")
async def create_alert(payload: AlertCreate, db: DbSession, principal: WritePrincipal) -> AlertOut:
    ...
    existing = (await db.execute(select(Alert).where(Alert.tenant_id == principal.tenant_id, Alert.fingerprint == fingerprint))).scalar_one_or_none()
    if existing:
    ...
    item = Alert(tenant_id=principal.tenant_id, fingerprint=fingerprint, first_triggered_at=now, last_triggered_at=now, **payload.model_dump())
    db.add(item)
    await db.flush()
    return AlertOut.model_validate(item)


@router.post("/alerts/{alert_id}/acknowledge", response_model=AlertOut, summary="Acknowledge an alert without deleting evidence")
...
@router.get("/sightings", response_model=ListResponse[SightingOut], summary="List entity sightings")
async def list_sightings(db: DbSession, principal: ReadPrincipal, limit: Annotated[int, Query(ge=1, le=500)] = 50, offset: Annotated[int, Query(ge=0)] = 0, entity_type: str | None = None) -> ListResponse[SightingOut]:
    ...
    total = await db.scalar(select(func.count()).select_from(stmt.subquery())) or 0
    rows = (await db.execute(stmt.order_by(Sighting.observed_at.desc()).limit(limit).offset(offset))).scalars().all()
    return ListResponse(data=[SightingOut.model_validate(x) for x in rows], page=Page(limit=limit, offset=offset, total=total, has_more=offset + limit < total), provenance=await build_provenance(db, sources=None))


@router.post("/sightings", response_model=SightingOut, status_code=status.HTTP_201_CREATED, summary="Ingest an observed entity sighting")
