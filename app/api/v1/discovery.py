...
_ALLOWED_TYPES: set[str] = {"vulnerability", "indicator", "actor", "ransomware_victim", "asset"}
...
@router.get("/search", response_model=SearchResponse, summary="Cross-entity intelligence search")
async def search(
    db: DbSession,
    principal: ReadPrincipal,
    q: Annotated[str, Query(min_length=2, max_length=256)],
    entity_type: Annotated[list[str] | None, Query(alias="type")] = None,
        ...
        rows = (
            await db.execute(
                ...
                .order_by(
                    ...
                    Indicator.last_seen.desc().nulls_last(),
                )
                .limit(per_type_limit)
            )
        ).scalars()
