...
SCOPE_CATALOGUE = {
    ...
    Scope.IOC: "Read indicator values and enrichment state.",
    Scope.ENROLL: "Enrol endpoint agents and submit heartbeats.",
    Scope.APIKEY_READ: "List masked API keys.",
    Scope.APIKEY_WRITE: "Mint and revoke bounded API keys.",
    Scope.REPORT_WRITE: "Request report generation.",
...
async def whoami(db: DbSession, principal: CurrentPrincipal) -> SessionOut:
    ...
    return SessionOut(
        ...
        note=(
            "This request is authenticated by a service API key. A user and role "
            "directory exists, but interactive OIDC sessions are not yet an "
...
async def access_scopes(db: DbSession, principal: ReadPrincipal) -> dict:
    ...
    return {
        ...
        "authentication_note": (
            "API keys remain the request principal until OIDC session verification "
            "is wired."
        ),
        "generated_at": datetime.now(UTC),
    }
...
async def access_principals(
    ...
    statement = select(ApiKey).where(ApiKey.tenant_id == principal.tenant_id)
    total = await db.scalar(
        select(func.count()).select_from(statement.subquery())
    ) or 0
    rows = (
        await db.execute(
            statement.order_by(ApiKey.created_at.desc()).limit(limit).offset(offset)
        )
    ).scalars().all()
    provenance = await build_provenance(db, sources=())
    provenance.note = (
        "API-key principals are listed here. Human users and role assignments are "
        "available from /users and /roles, but are not yet request-authentication "
        "principals."
    )
    return ListResponse(
...
async def sharing_groups(
    ...
    statement = select(IntelCollection).where(
    ...
    rows = (
        await db.execute(
            statement.order_by(IntelCollection.name).limit(limit).offset(offset)
        )
    ).scalars().all()
    provenance = await build_provenance(db, sources=None)
    provenance.note = (
        "Shared means tenant-internal. Cross-tenant publication remains unavailable."
    )
    return ListResponse(
