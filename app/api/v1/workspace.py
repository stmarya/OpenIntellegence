...
from app.db.platform_models import Role,RoleAssignment,User
...
async def sharing_groups(db:DbSession,p:ReadPrincipal,limit:Annotated[int,Query(ge=1,le=200)]=50,offset:Annotated[int,Query(ge=0)]=0)->ListResponse[SharingGroupOut]:
 stmt=select(IntelCollection).where(IntelCollection.tenant_id==p.tenant_id,IntelCollection.is_shared.is_(True));total=await db.scalar(select(func.count()).select_from(stmt.subquery())) or 0;rows=(await db.execute(stmt.order_by(IntelCollection.name).limit(limit).offset(offset))).scalars().all();provenance=await build_provenance(db,sources=None);provenance.note="Shared means tenant-internal. Cross-tenant publication remains unavailable."
 return ListResponse(data=[SharingGroupOut.model_validate(x) for x in rows],page=Page(limit=limit,offset=offset,total=total,has_more=offset+limit<total),provenance=provenance)
