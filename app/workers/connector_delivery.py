"""Connector outbox worker with leases, receipts, bounded retry, and no API secrets."""
...
class SlackConnector:
    ...
    async def deliver(self, item: AutomationOutbox) -> DeliveryReceipt:
        text = item.payload.get("step_payload", {}).get("text") or item.payload.get("run_context", {}).get("summary")
        if not text:
...
def registry(settings: Settings) -> dict[str, Connector]:
    ...
    if settings.slack_webhook_url:
        result["slack.notify"] = SlackConnector(settings.slack_webhook_url.get_secret_value(), settings.connector_delivery_timeout_seconds)
    if settings.jira_base_url and settings.jira_email and settings.jira_api_token:
        result["jira.issue.create"] = JiraConnector(settings.jira_base_url, settings.jira_email, settings.jira_api_token.get_secret_value(), settings.connector_delivery_timeout_seconds)
    if settings.siem_webhook_url:
        token = settings.siem_webhook_token.get_secret_value() if settings.siem_webhook_token else None
        result["siem.push"] = SiemWebhookConnector(settings.siem_webhook_url.get_secret_value(), token, settings.connector_delivery_timeout_seconds)
    return result
...
class DeliveryWorker:
    ...
    async def deliver(self, session: AsyncSession, item: AutomationOutbox) -> None:
        ...
        if connector is None:
            receipt = DeliveryReceipt(False, error=f"No enabled connector for action {item.action}.")
        else:
            try:
        ...
        if receipt.delivered:
            item.state, item.delivered_at, item.delivery_result, item.last_error = "delivered", datetime.now(UTC), {"remote_id": receipt.remote_id, "detail": receipt.detail or {}}, None
        elif receipt.retryable and item.attempts < self.settings.connector_max_attempts:
            item.state, item.available_at, item.last_error = "retry", datetime.now(UTC) + retry_delay(item.attempts), receipt.error
        else:
            item.state, item.delivery_result, item.last_error = "dead_letter", {"error": receipt.error, "detail": receipt.detail or {}}, receipt.error
        await session.commit()
