"""Cross-cutting identity, graph, search, command, AI, and detection models."""
...
class ConnectorCheckpoint(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "source", name="uq_connector_checkpoint"),
    )
...
class AgentCommand(Base, TimestampMixin):
    ...
    intent_id: Mapped[str] = mapped_column(
        UuidType,
        ForeignKey("endpoint_intents.id", ondelete="CASCADE"),
        unique=True,
        nullable=False,
    )
    ...
    state: Mapped[str] = mapped_column(
        String(24), default="available", nullable=False, index=True
    )
...
class AiEvaluation(Base, TimestampMixin):
    ...
    expected_refs: Mapped[list[str]] = mapped_column(
        StrArray, default=list, nullable=False
    )
    actual_refs: Mapped[list[str]] = mapped_column(
        StrArray, default=list, nullable=False
    )
...
class DetectionRule(Base, TimestampMixin):
    ...
    attack_techniques: Mapped[list[str]] = mapped_column(
        StrArray, default=list, nullable=False
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "name",
            "version",
            name="uq_detection_rule_version",
        ),
    )
