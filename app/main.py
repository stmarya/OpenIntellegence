...
DESCRIPTION = """OpenIntelligence ingests cyber-threat intelligence, correlates it
with endpoint inventory, and exposes a tenant-scoped REST API. API clients use
scoped keys; endpoint agents use mTLS. List responses disclose provenance and
degraded sources."""


@asynccontextmanager
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    """Initialize optional runtime dependencies and release resources."""
...
def create_app() -> FastAPI:
    ...
    async def telemetry(request: Request, call_next):
        ...
        request_id = request.headers.get("X-Request-ID") or token_hex(16)
        try:
            response = await call_next(request)
        except Exception:
            route = getattr(request.scope.get("route"), "path", "unmatched")
            observe(request.method, route, 500, time.perf_counter() - started)
            log.exception("request_failed", request_id=request_id, route=route)
            raise
        elapsed = time.perf_counter() - started
        route = getattr(request.scope.get("route"), "path", "unmatched")
        observe(request.method, route, response.status_code, elapsed)
